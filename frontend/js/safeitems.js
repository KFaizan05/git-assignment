...
(async function initSafeItems() {
  "use strict";

  // Hydrate the client cache from the server before touching any sync getter.
  ...
  safeList.addEventListener("click", async (event) => {
    ...
    if (action === "fav" && id) {
      ...
      if (!scan) return;
      try {
        await profileStorage.updateCurrentScan(id, { savedToSafe: !scan.savedToSafe });
      } catch (err) {
        console.warn("safeitems: favorite toggle failed –", err && err.message);
      }
      render();
      return;
    }
    ...
    if (action === "view" && id) {
      ...
      // Tag the origin so the Results page back button returns to My Safe
      // Items instead of falling back to the generic history stack.
      sessionStorage.setItem("revisitOrigin", "SafeItemsPage.html");
