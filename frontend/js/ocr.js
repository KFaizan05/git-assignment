...
// certainly hallucinated garbage and gets stripped.
...
async function runOCRFlow() {
  try {
    ...
    // --- Tesseract.js fallback. Runs locally so the app still works if
    // OCR.space is down, rate-limited, or the user is offline.
    const needLocalFallback =
      candidates.length === 0 ||
      scoreIngredientOcr(best.text) < 40 ||
      !looksLikeIngredients(sanitizeText(best.text));

    if (needLocalFallback) {
      try {
        setStepState(stepRead, "active", "Reading ingredients (on-device)...");
        const tessText = await runTesseractBest(processedDataUrl);
        if (tessText) {
          candidates.push({ text: tessText, source: "tesseract" });
          best = pickBestOcrCandidate(candidates);
        }
      } catch (tessErr) {
    ...
    // --- Finalize: sanitize, sanity-check, and hand off to Results.
    const finalText = sanitizeText(best.text || "");

    if (!finalText || !looksLikeIngredients(finalText)) {
      setStepState(stepRead, "pending", "No text found");
      setStepState(stepSafety, "pending", "Analyzing safety");
      sessionStorage.setItem("ocrText", "");
      showErrorModal({
        icon: "📷",
        title: "Unreadable scan",
        body: "We couldn't read the ingredients on that image. Please retake the photo with the label clearly visible and in focus.",
        retryText: "Try Again",
        cancelText: "Cancel",
        onRetry:  () => { window.location.href = "CropPage.html"; },
        onCancel: () => { window.location.href = "ScanPage.html"; }
    ...
    setStepState(stepRead, "completed", "Ingredients read");
    setStepState(stepSafety, "active", "Analyzing safety");
    sessionStorage.setItem("ocrText", finalText);

    // Tiny delay so the final step's "active" state actually paints before
    // the navigation — otherwise the user just sees a flash of the page.
    setTimeout(() => { window.location.href = "ResultsPage.html"; }, 200);
  } catch (err) {
    console.error("OCR flow failed:", err);
    setStepState(stepRead, "pending", "Something went wrong");
    setStepState(stepSafety, "pending", "Analyzing safety");
    showErrorModal({
      icon: "⚠",
      title: "Something went wrong",
      body: (err && err.message) || "We hit an unexpected error while reading that image.",
      retryText: "Try Again",
      cancelText: "Cancel",
      onRetry:  () => { window.location.href = "CropPage.html"; },
      onCancel: () => { window.location.href = "ScanPage.html"; }
...
// Kick off the flow as soon as the script loads. If the user landed on
// ParsingPage without a cropped image in sessionStorage, bail back to the
// scanner so we don't stall forever on a blank state.
if (!hasCroppedImage) {
  window.location.replace("ScanPage.html");
} else {
  runOCRFlow();
}